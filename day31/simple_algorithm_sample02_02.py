...
def select_sort_verbose(items, comp=lambda x, y: x < y):
    ...
    for i in range(n - 1):
        ...
        for j in range(i + 1, n):
            ...
            #   第一次比较：j=1, min_index=0，比较 1<3=True → min_index=1
            #   第二次比较：j=2, min_index=1，比较 4<1=False → min_index=1
            #   第三次比较：j=3, min_index=1，比较 2<1=False → min_index=1
            #   交换 items[0]=3 和 items[1]=1
            #   第一轮排序输出的结果：[1, 3, 4, 2]
            #
            # 第二轮比较 (i=1):
            #   初始化：当 i=1, j=2, min_index=1 时
            #   第一次比较：j=2, min_index=1，比较 4<3=False → min_index=1
            #   第二次比较：j=3, min_index=1，比较 2<3=True → min_index=3
            #   交换 items[1]=3 和 items[3]=2
            #   第二轮排序输出的结果：[1, 2, 4, 3]
            #
            # 第三轮比较 (i=2):
            #   初始化：当 i=2, j=3, min_index=2 时
            #   第一次比较：j=3, min_index=2，比较 3<4=True → min_index=3
        ...
        # 将最优元素交换到当前位置i
        # 示例：i=0, min_index=1 时，交换位置0和位置1的元素
...
if __name__ == "__main__":
    ...
    # 调用选择排序函数，comp=lambda x, y: x < y 表示升序排序
    sorted_nums = select_sort_verbose(nums, comp=lambda x, y: x < y)
    print("\n最终排序结果:", sorted_nums)
    
    # 补充说明：如果要改为降序排序，使用 comp=lambda x, y: x > y
    print("\n" + "="*50)
    print("补充说明：")
